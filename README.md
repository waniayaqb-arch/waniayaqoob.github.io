#  Wania Yaqoob

##  Objective

Aspiring Computer Science student focused on developing strong technical and problem-solving skills. Passionate about gaining hands-on experience through real-world software projects and continuously expanding knowledge in the field of computing.

---

## Contact Information

*  Address: Green Villas Snobercity, Lane 05, House G-60, Adyala Road, Rawalpindi
*  Email: [waniayaqb@gmail.com](mailto:waniayaqb@gmail.com)

---

##  Skills

###  Programming Languages

* C
* C++
* Java
* Python
* Assembly Language

###  Web Technologies

* HTML
* JavaScript

###  Tools & Technologies

* Dev C++
* VS Code
* Oracle Database
* Anaconda Navigator (Jupyter Notebook)

---

## 🚀 Projects

* 🔹 **Tower of Hanoi**
  Implementation of recursive problem-solving approach.

* 🔹 **Traffic Light Control System**
  Simulation of automated traffic signal system.

* 🔹 **Wedding Hall Management System (Database)**
  Database-based system for managing bookings and operations.

---

##  Education

* **Bachelor of Computer Science (BCS)**
  Fatima Jinnah Women University (2024 – Present)

* **Intermediate (ICS)**
  Army Public School & College Askari XIV (2024) — Grade: A

* **Matriculation**
  Army Public School & College Askari XIV (2022) — Grade: A1

---

## 💬 Communication Skills

* Effective verbal and written communication
* Ability to present ideas clearly
* Strong teamwork and collaboration in academic projects

---

##  Leadership

* Demonstrated ability to take initiative in academic projects
* Experience in managing tasks and guiding team members toward successful completion

---

##  Currently Learning

* Advanced programming concepts
* Software development practices
* Problem-solving and algorithm design

---

##  Goals

* Build impactful software projects
* Gain practical industry experience
* Continuously improve technical expertise

---

 *Feel free to explore my repositories and connect with me!*
